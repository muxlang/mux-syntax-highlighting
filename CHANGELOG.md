...
- **An identifier can begin with an underscore.** `identifiers.pattern` in the
  ...
  `_123` and `__` highlight as names. A bare `_` is unchanged and keeps its own
  rule - it is the match wildcard and the unused-parameter marker, which is why
  the second alternative requires at least one character after the underscore.
  The pattern is wrapped in a non-capturing group because the TextMate generator
  interpolates it between `\b` anchors, where a bare top-level alternation would
  bind only to the first branch. Closes muxlang/mux-context#46.

## [0.5.0] - 2026-07-13

First changelog for `mux-syntax-highlighting` as a standalone, independently
versioned repo, extracted from the former monorepo. This repo owns the canonical
syntax spec (`shared/syntax-matrix.json`) and the TextMate grammar / VSCode
extension; the extension version lives in
`textmate-mux/vscode-language-mux/package.json` and is released on its own cadence.

### Added
- **`where` keyword**: Added the `where` keyword to the canonical syntax matrix so
  all downstream highlighters pick it up (#6).

### Changed
- **Single canonical syntax source**: Merged the two syntax specs into one canonical
  `syntax-matrix.json`, removing the drifted `editor-support/spec/definitions.json`.
- **Editor-support generator ported to Node**: Rewrote the editor-support config
  generator from Python to Node (`build-editor-support.js`).
- **Standalone repo setup**: Established as an independent repo with CLAUDE.md/AGENTS
  linking the muxlang/mux-context hub.

### Fixed
- **`DotDot` operator description**: Corrected to describe the rest pattern, not a
  range (#7).
